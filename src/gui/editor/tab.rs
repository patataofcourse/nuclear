...
impl Widget for Tab<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        ui.vertical(|ui| {
            ...
            let text = WidgetText::from(format!("{} ({})", self.name, self.editor_type))
                .into_galley(ui, Some(false), f32::INFINITY, FontSelection::Default);
            let button_size =
            ...
            if ui.is_rect_visible(rect) {
                ...
                // X
                let x_text = WidgetText::from("x").into_galley(
                    ui,
                    None,
                    f32::INFINITY,
